...
围绕 `QuantInvestor` 单一主线，系统采用**双层智能体架构**：确定性控制层负责硬约束、风控否决与组合构建，LLM 审阅层提供定性评判与结构化辩论。审阅层严格为 advisory-only，永远不覆盖控制层的硬否决。覆盖 A 股与美股市场，完整支持从基本面分析到投资报告生成的全流程管道。
...
| **Pipeline Result** | `quant_investor.pipeline.QuantInvestorPipelineResult` |
...
多分支审阅结果进入确定性控制链（Research Agents -> RiskGuard -> ICCoordinator -> PortfolioConstructor -> NarratorAgent），审阅层只提供 advisory 信号。支持 OpenAI、Anthropic Claude、DeepSeek、Google Gemini 等多家提供方，无 API Key 时自动降级为纯算法模式。
...
| `TUSHARE_URL` | Tushare 代理或自定义 API 地址（可选） |
| `DB_PATH` | 本地数据库路径 |
| `LOG_LEVEL` | 日志级别 |
...
| `Research Agents -> RiskGuard -> ICCoordinator -> PortfolioConstructor -> NarratorAgent` | 当前结构化控制链 |
...
| 命令 | 说明 |
|------|------|
| `quant-investor research run` | 运行单一主线研究流程 |
| `quant-investor web` | 启动研究工作台后端入口 |
| `./run_web.sh` | 启动本地前端工作流 |
| `pytest tests/ -v` | 运行完整测试套件 |
