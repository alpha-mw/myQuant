...
class ICCoordinator(BaseAgent):
    ...
    def run(self, payload: Mapping[str, Any]) -> ICDecision:
        ...
        ic_hints = self._normalize_ic_hints(envelope.get("ic_hints"))
        ...
        if ic_hints:
            hinted_score = ic_hints.get("score")
            if hinted_score is not None:
                raw_score = self.clamp(raw_score * 0.7 + float(hinted_score) * 0.3, -1.0, 1.0)
            hinted_confidence = ic_hints.get("confidence")
            if hinted_confidence is not None:
                raw_confidence = self.clamp(
                    raw_confidence * 0.7 + float(hinted_confidence) * 0.3,
                    0.0,
                    1.0,
                )
        raw_action = self.score_to_action(raw_score)
        if ic_hints.get("action"):
            raw_action = self.more_restrictive_action(
                raw_action,
                str(ic_hints["action"]).strip().lower(),
            )
        ...
        rationale_points.extend(ic_hints.get("rationale_points", [])[:2])
        ...
        return ICDecision(
            ...
            metadata={
                ...
                "llm_hint_applied": bool(ic_hints),
                "llm_hint_action": str(ic_hints.get("action", "")),
                "llm_hint_score": float(ic_hints.get("score", raw_score)),
            },
        )
    ...
    @staticmethod
    def _normalize_ic_hints(payload: Any) -> dict[str, Any]:
        if payload is None:
            return {}
        if not isinstance(payload, Mapping):
            raise TypeError("ic_hints 必须是 Mapping[str, Any]")
        result = dict(payload)
        if "rationale_points" in result and not isinstance(result["rationale_points"], list):
            result["rationale_points"] = []
        return result
