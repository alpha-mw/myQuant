...
def persist_macro_observer(
    ...
    *,
    ...
    if generation_slug and not re.fullmatch(r"[A-Za-z0-9_.-]+", generation_slug):
...
def load_macro_observations(path: str | Path) -> list[dict[str, Any]]:
    rows, _ = load_macro_observation_generation(path)
    return rows


def load_macro_observation_generation(path: str | Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
