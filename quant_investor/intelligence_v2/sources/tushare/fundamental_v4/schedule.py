...
def _symbols(values: Sequence[str]) -> list[str]:
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalV4ContractError("symbols must be a sequence")
    rows = list(values)
    if not rows or len(rows) > 10_000:
        raise FundamentalV4ContractError("symbol scope cardinality is invalid")
    normalized: list[str] = []
    for value in rows:
        if type(value) is not str or not value or not value.isascii():
            raise FundamentalV4ContractError("symbol scope contains an invalid value")
    ...
    if normalized != expected or len(normalized) != len(set(normalized)):
        raise FundamentalV4ContractError("symbol scope must be ASCII-sorted unique")
    return normalized


def _open_sessions(values: Sequence[str], *, start: str, end: str) -> list[str]:
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalV4ContractError("daily_open_sessions must be a sequence")
    ...
    if (
        ...
        raise FundamentalV4ContractError("daily open-session closure is invalid")
    return rows
...
def _validated_endpoint_plans(
    ...
    if type(value) is not dict or set(value) != set(SOURCE_ENDPOINTS):
        raise FundamentalV4ContractError("endpoint plan set is invalid")
    ...
    for table in sorted(SOURCE_ENDPOINTS):
        ...
        if (
            ...
            raise FundamentalV4ContractError("endpoint plan authority mismatch")
        plans[table] = plan
...
def _baseline_empty_keyset(
    ...
    *,
    ...
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalV4ContractError("baseline_empty_partition_keyset must be a sequence")
    rows = list(values)
    ...
    if (
        ...
        raise FundamentalV4ContractError("baseline empty partition keyset is invalid")
    return rows


@fundamental_v4_contract
def build_fundamental_request_plan_v4(
    *,
    as_of: str,
    pit_cutoff: str,
    ...
    if cutoff > created or cutoff[:10].replace("-", "") > target:
        raise FundamentalV4ContractError("PIT cutoff is future-dated")
    ...
    if (
        ...
        raise FundamentalV4ContractError("market closure contains future evidence")
...
@fundamental_v4_contract
def validate_fundamental_request_plan_v4(
    ...
    *,
    ...
    if value.get("version") != REQUEST_PLAN_V4:
        raise FundamentalV4ContractError("Fundamental request plan version mismatch")
    expected = build_fundamental_request_plan_v4(
        as_of=value["as_of"],
    ...
    if value != expected:
        raise FundamentalV4ContractError("Fundamental request plan replay mismatch")
    return value


@fundamental_v4_contract
def build_fundamental_execution_closure_v4(
    *,
    ...
    validated = validate_fundamental_request_plan_v4(
        plan,
        endpoint_plans=endpoint_plans,
    )
    ...
    if created < validated["created_at"]:
        raise FundamentalV4ContractError("execution closure predates request plan")
    return seal(
        {
            **common_fields(timestamp_value=created),
            "created_at": created,
...
@fundamental_v4_contract
def validate_fundamental_execution_closure_v4(
    document: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if value.get("version") != EXECUTION_CLOSURE_V4:
        raise FundamentalV4ContractError("execution closure version mismatch")
    expected = build_fundamental_execution_closure_v4(
        plan=value["request_plan"],
        endpoint_plans=value["endpoint_plans"],
        created_at=value["created_at"],
    )
    if value != expected:
        raise FundamentalV4ContractError("execution closure replay mismatch")
    return value
