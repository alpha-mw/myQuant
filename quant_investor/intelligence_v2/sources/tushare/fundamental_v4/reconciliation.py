...
def _documents(value: Any, *, label: str, maximum: int) -> list[Mapping[str, Any]]:
    ...
    if any(type(row) is not dict for row in rows):
        raise FundamentalV4ContractError(f"{label} contains a non-object")
    return rows
...
def _physical_receipts(
    ...
    *,
    ...
    if sorted(identities) != sorted(expected) or len(identities) != len(set(identities)):
        raise FundamentalV4ContractError("physical request keyset is not closed")
...
def _logical_coverages(
    ...
    *,
    ...
    if sorted(identities) != sorted(expected) or len(identities) != len(set(identities)):
        raise FundamentalV4ContractError("logical coverage keyset is not closed")
...
def _raw_evidence(
    ...
    *,
    ...
    if (
        ...
        raise FundamentalV4ContractError(f"{lane} raw evidence set is invalid")
...
def _output_refs(value: Mapping[str, Mapping[str, Any]], *, reconciled_at: str) -> dict:
    if type(value) is not dict or set(value) != set(_OUTPUT_PATHS):
        raise FundamentalV4ContractError("comparison output ref set is invalid")
    ...
    for name, required_path in _OUTPUT_PATHS.items():
        ...
        if item["relative_path"] != required_path or item["available_at"] > reconciled_at:
            raise FundamentalV4ContractError("comparison output ref is not durable or timely")
        result[name] = item
    return result
...
def _derived(value: Mapping[str, Mapping[str, Any]]) -> tuple[dict, bool]:
    if type(value) is not dict or set(value) != set(_DERIVED_TABLES):
        raise FundamentalV4ContractError("derived fingerprint set is invalid")
    result: dict[str, dict[str, str]] = {}
...
def build_fundamental_reconciliation_receipt(
    *,
    ...
    if reconciled < validated_plan["created_at"]:
        raise FundamentalV4ContractError("reconciliation predates its request plan")
    ...
    body = {
        **common_fields(timestamp_value=reconciled),
        ...
        "version": RECONCILIATION_RECEIPT_V1,
...
def validate_fundamental_reconciliation_receipt(
    ...
    **closure: Any,
    ...
    if value.get("version") != RECONCILIATION_RECEIPT_V1:
        raise FundamentalV4ContractError("reconciliation receipt version mismatch")
    expected = build_fundamental_reconciliation_receipt(
        **closure,
        reconciled_at=value["timestamp"],
    )
    if value != expected:
        raise FundamentalV4ContractError("reconciliation receipt replay mismatch")
    return value
