"""Offline compiler and replay validator for shadow-only V17 v4 Deep v2."""
...
ASSESSMENT_VERSION: Final = "myquant.v17.v4.deep-assessment-manifest.v1"
OFFICIAL_EVIDENCE_V2: Final = "myquant.v17.v4.official-evidence.v2"
ISSUER_DOSSIER_V2: Final = "myquant.v17.v4.issuer-dossier.v2"
EVENT_SCAN_V2: Final = "myquant.v17.v4.event-scan.v2"
DEEP_BUNDLE_V2: Final = "myquant.v17.v4.deep-evidence-bundle.v2"
...
class DeepV2Error(RuntimeError):
    """Raised when a Deep v2 compiler or replay gate fails closed."""

    exit_code = 2


def _blocked(reason: str) -> NoReturn:
    raise DeepV2Error(f"V17_V4_DEEP_V2_BLOCKED:{reason}")
...
def _artifact_ref(
    ...
    *,
    ...
    except (IdentityContractError, ValueError):
...
def _policy() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    expected = {
        "authority",
        "buy_veto_signal_threshold",
        "deep_penalty_scale",
        "max_deep_penalty",
        "missing_evidence_action",
        "module_order",
        "module_red_flag_conclusion_veto",
        "module_weights",
        "protocol_version",
        "semantic_sha256",
        "severe_red_flags",
    ...
    if (
        ...
        or sum(
            (_decimal(weights[name], label=f"weight.{name}") for name in order),
            Decimal("0"),
        )
        != Decimal("1")
...
def _load_exact_artifact(
    ...
    *,
    ...
    try:
        raw = artifact_loader({str(key): str(value) for key, value in reference.items()})
        ...
        raise DeepV2Error("V17_V4_DEEP_V2_BLOCKED:artifact_readback") from exc
    if (
    ...
    *,
    ...
    for module in modules:
        try:
            ...
            raise DeepV2Error("V17_V4_DEEP_V2_BLOCKED:module_evidence") from exc
        result.append(
            {
...
def score_dossier_v2(
    dossier: Mapping[str, Any],
    event_scan: Mapping[str, Any],
) -> tuple[Decimal, bool]:
    """Recompute the fixed-weight Deep signal and veto truth table."""
    ...
    if (
        ...
        or [row.get("module_id") for row in modules] != policy["module_order"]
    ):
    ...
    severe = bool(flags & set(policy["severe_red_flags"]))
    module_veto = any(row["conclusion"] == "RED_FLAG" for row in modules)
    threshold_veto = signal <= _decimal(
        policy["buy_veto_signal_threshold"],
        label="buy_veto_signal_threshold",
    )
    return signal, bool(severe or module_veto or threshold_veto)
...
class _DeepV2Writer(GovernedStore):
    def __init__(self, workspace_root: str, *, run_id: str) -> None:
        super().__init__(workspace_root)
        self._root = RUN_ROOT / require_opaque_id(run_id, label="run_id") / "deep_v2"
    ...
    def _canonical_path(
        ...
        if path != self._root and self._root not in path.parents:
            raise SourceStorageError("Deep v2 writer path is outside its immutable run root")
        return path
...
def compile_deep_v2(
    ...
    *,
    ...
    """Compile one complete Top24 assessment manifest without external calls."""
    ...
    try:
        manifest_raw = reader.read(assessment_manifest_path, expected)
        ...
        raise DeepV2Error("V17_V4_DEEP_V2_BLOCKED:assessment_manifest") from exc
    if type(manifest) is not dict or manifest["version"] != ASSESSMENT_VERSION:
    ...
    if (
        manifest["run_id"] != fusion["run_id"]
        or created < manifest["cutoff"]
    ...
    for assessment, top_row in zip(manifest["rows"], fusion["rows"], strict=True):
        ...
        for raw_document in assessment["raw_documents"]:
            try:
                ...
                raise DeepV2Error("V17_V4_DEEP_V2_BLOCKED:raw_evidence_readback") from exc
            ...
            evidence = seal_semantic(
                {
                    ...
                    "symbol": symbol,
                    "version": OFFICIAL_EVIDENCE_V2,
                }
            )
            validate_artifact(evidence)
            path = (
                f"{RUN_ROOT}/{manifest['run_id']}/deep_v2/"
                f"{symbol}/official/{raw_document['evidence_id']}.json"
            )
            reference = _artifact_ref(evidence, relative_path=path)
        ...
        dossier = seal_semantic(
            {
                ...
                "created_at": created,
                "cutoff": manifest["cutoff"],
                "dossier_id": f"deep-v2-dossier-{stable_id}",
                "modules": modules,
                ...
                "symbol": symbol,
                "version": ISSUER_DOSSIER_V2,
            }
        )
        validate_artifact(dossier)
        dossier_path = f"{RUN_ROOT}/{manifest['run_id']}/deep_v2/" f"{symbol}/issuer_dossier.json"
        ...
        event = seal_semantic(
            {
                ...
                "official_evidence_refs": official_refs,
                "protocol_version": PROTOCOL_VERSION,
                "scan_id": f"deep-v2-event-{stable_id}",
                "strategy_id": manifest["strategy_id"],
                "symbol": symbol,
                "version": EVENT_SCAN_V2,
            }
        )
        validate_artifact(event)
        event_path = f"{RUN_ROOT}/{manifest['run_id']}/deep_v2/" f"{symbol}/event_scan.json"
        ...
        signal, buy_veto = score_dossier_v2(dossier, event)
    ...
    bundle_id = (
        "deep-v2-bundle-"
        + hashlib.sha256(
            f"{manifest['run_id']}:{manifest['request_id']}".encode("ascii")
        ).hexdigest()[:24]
    )
    bundle = seal_semantic(
        {
            ...
            "strategy_id": manifest["strategy_id"],
            "version": DEEP_BUNDLE_V2,
        }
    )
    validate_artifact(bundle)
    bundle_path = f"{RUN_ROOT}/{manifest['run_id']}/deep_v2/" "deep_evidence_bundle.v2.json"
    ...
    replayed, replayed_fusion, _ = revalidate_deep_v2_bundle(
        bundle_ref,
        artifact_loader=staged_loader,
    )
    if replayed != bundle or replayed_fusion != fusion:
        _blocked("prepublication_replay")
    writer = _DeepV2Writer(workspace_root, run_id=manifest["run_id"])
    writer.initialize()
    for path, raw in staged:
        writer.write_exact_once(path, raw)
    write_result = writer.write_exact_once(bundle_path, bundle_raw)
    return {
        "authority": dict(NO_AUTHORITY),
        ...
        "run_id": manifest["run_id"],
        "shadow_only": True,
        "status": "DEEP_V2_COMPILED",
        "strategy_id": manifest["strategy_id"],
    }


def revalidate_deep_v2_bundle(
    bundle_ref: Mapping[str, Any],
    *,
    artifact_loader: Callable[[Mapping[str, str]], bytes],
) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    """Reopen the Deep v2 bundle, raw evidence, dossier and policy closure."""
    ...
    for bundle_row, assessment, top_row in zip(
        ...
        for reference in bundle_row["official_evidence_refs"]:
            document = _load_exact_artifact(
                reference,
                expected_version=OFFICIAL_EVIDENCE_V2,
                artifact_loader=artifact_loader,
            )
            try:
                raw = artifact_loader(
                ...
                raise DeepV2Error("V17_V4_DEEP_V2_BLOCKED:raw_replay") from exc
            if (
        ...
        dossier = _load_exact_artifact(
            ...
            expected_version=ISSUER_DOSSIER_V2,
            artifact_loader=artifact_loader,
        )
        event = _load_exact_artifact(
            bundle_row["event_scan_ref"],
            expected_version=EVENT_SCAN_V2,
            artifact_loader=artifact_loader,
        )
        ...
        expected_flags = list(assessment["event_flags"])
        if (
