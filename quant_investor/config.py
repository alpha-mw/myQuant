"""Shared runtime and market-maintenance configuration."""
...
def _reject_retired_env_keys() -> None:
    configured = sorted(key for key in RETIRED_ENV_KEYS if key in os.environ)
    if configured:
        raise RuntimeError(
...
class Config:
    ...
    FUNNEL_PROFILE: str = (
        str(os.environ.get("FUNNEL_PROFILE", "momentum_leader") or "momentum_leader")
        .strip()
        .lower()
    )
    ...
    PIT_UNIVERSE_SOURCE_ROOT: str = _env_str(
        "PIT_UNIVERSE_SOURCE_ROOT", "data/parquet/cn/reference"
    )
