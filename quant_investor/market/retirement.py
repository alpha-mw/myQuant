...
JOURNAL_SCHEMA = "retirement-purge-journal.v1"
RECEIPT_SCHEMA = "retirement-purge-receipt.v1"
...
REQUIRED_SCHEDULE_NAMES = (*ACTIVE_SCHEDULE_NAMES, *LEGACY_SCHEDULE_NAMES)
...
def _fsync_dir(path: Path) -> None:
    ...
    try:
        if not stat.S_ISDIR(os.fstat(fd).st_mode):
            raise RetirementError(f"fsync target is not a real directory: {path}")
        os.fsync(fd)
    finally:
        os.close(fd)
...
def _expected_quarantine(
    ...
    return repo_root / "results" / f".{source.name}.retirement-purge-{cutover_id}-{nonce}"
...
def _entry_stat(directory_fd: int, name: str) -> os.stat_result:
    try:
        ...
        raise RetirementError(f"purge tree entry changed: {name}: {exc}") from exc
...
def _open_child_directory(
    ...
    try:
        ...
        raise RetirementError(f"purge tree directory is unsafe: {name}: {exc}") from exc
    child_stat = os.fstat(child_fd)
    if (
        not stat.S_ISDIR(child_stat.st_mode)
        or (child_stat.st_dev, child_stat.st_ino) != (expected.st_dev, expected.st_ino)
    ):
        os.close(child_fd)
        raise RetirementError(f"purge tree directory identity drifted: {name}")
    return child_fd


def _validate_tree_fd(directory_fd: int, *, expected_device: int) -> None:
    for name in sorted(os.listdir(directory_fd)):
        item = _entry_stat(directory_fd, name)
        if item.st_dev != expected_device:
            raise RetirementError(f"purge tree crosses a filesystem boundary: {name}")
        if stat.S_ISDIR(item.st_mode):
            child_fd = _open_child_directory(directory_fd, name, item)
            try:
                _validate_tree_fd(child_fd, expected_device=expected_device)
            finally:
                os.close(child_fd)
            continue
        if not stat.S_ISREG(item.st_mode):
            raise RetirementError(f"purge tree contains a non-regular entry: {name}")
        if item.st_nlink != 1:
            raise RetirementError(f"purge tree contains a hard-linked file: {name}")
...
def _delete_tree_fd(
    ...
    *,
    ...
    for name in sorted(os.listdir(directory_fd)):
        ...
        _call_hook(event_hook, "DELETE_ENTRY_UNLINKED", root)
...
def _safe_rmtree(
    ...
    *,
    ...
    try:
        root_stat = _entry_stat(parent_fd, path.name)
        if (
            not stat.S_ISDIR(root_stat.st_mode)
            or root_stat.st_dev != expected_device
            or root_stat.st_ino != expected_inode
        ):
            raise RetirementError("quarantine identity drifted before deletion")
        root_fd = _open_child_directory(parent_fd, path.name, root_stat)
        try:
            _validate_tree_fd(root_fd, expected_device=expected_device)
            _delete_tree_fd(
                root_fd,
                expected_device=expected_device,
                root=root,
...
def _validate_tree_path(
    ...
    *,
    ...
    try:
        ...
        raise RetirementError(f"purge source tree is unsafe: {path}: {exc}") from exc
    try:
        ...
        if (
            ...
            raise RetirementError("purge source tree identity drifted")
        _validate_tree_fd(root_fd, expected_device=expected_device)
    finally:
        os.close(root_fd)


def _rename_exact_directory(
    source: Path,
    quarantine: Path,
    *,
    expected_device: int,
    ...
    if source.parent != quarantine.parent:
        raise RetirementError("source and quarantine must share the exact results parent")
    flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
    parent_fd = os.open(source.parent, flags)
    try:
        source_stat = _entry_stat(parent_fd, source.name)
        if (
            not stat.S_ISDIR(source_stat.st_mode)
            or source_stat.st_dev != expected_device
            or source_stat.st_ino != expected_inode
        ):
            raise RetirementError("source identity drifted before rename")
        try:
            os.stat(quarantine.name, dir_fd=parent_fd, follow_symlinks=False)
        except FileNotFoundError:
            pass
        else:
            raise RetirementError("quarantine appeared before rename")
        os.rename(
            source.name,
            quarantine.name,
    ...
    with _exclusive_journal_lock(path):
        ...
        for relative_path in SOURCE_RELATIVE_PATHS:
            ...
            _validate_tree_path(
                ...
                expected_inode=source_stat.st_ino,
            )
    ...
    *,
    ...
    with _exclusive_journal_lock(path):
        ...
        if not crossed_boundary:
            for root in journal["roots"]:
                _validate_tree_path(
                    Path(str(root["source_path"])),
                    expected_device=int(root["source_st_dev"]),
                    expected_inode=int(root["source_st_ino"]),
                )
