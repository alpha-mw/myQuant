...
def _runtime_profile_dir(
    ...
    *,
    ...
    profile_anchor = report_paths.get("trade_report") or report_paths.get("summary_report")
    if profile_anchor:
...
def write_runtime_profile_artifacts(
    *,
    ...
    runtime_profile_json = profile_dir / f"{market}_Runtime_Profile_{profile_timestamp}.json"
    runtime_profile_md = profile_dir / f"{market}_Runtime_Profile_{profile_timestamp}.md"
...
def _fundamental_counterfactual_manifest_meta(
    ...
    actual_variant = "without_dossier" if alternative_variant == "with_dossier" else "with_dossier"
...
def write_analysis_run_manifest(
    *,
    ...
    """Persist an immutable, private research-input manifest for a completed run."""
    ...
    meta_hash = hashlib.sha256(_json_bytes(normalized_meta)).hexdigest()
    now_utc = datetime.now(timezone.utc)
    generated_at = now_utc.isoformat()
    run_id = f"{str(market).upper()}_{now_utc.strftime('%Y%m%dT%H%M%S%fZ')}"
    payload: dict[str, Any] = {
        ...
        "analysis_meta_sha256": meta_hash,
    ...
    if target.exists() or target.is_symlink():
        raise FileExistsError(f"analysis run manifest is immutable and already exists: {target}")
    ...
    if companion is not None:
        ...
        alternative_payload: dict[str, Any] = {
            ...
            "analysis_meta_sha256": hashlib.sha256(_json_bytes(alternative_meta)).hexdigest(),
            "analysis_meta": alternative_meta,
        }
        alternative_payload["manifest_sha256"] = hashlib.sha256(
            _json_bytes(alternative_payload)
        ).hexdigest()
        companion_target = run_dir / f"analysis_run_manifest.{alternative_variant}.v1.json"
        _atomic_write_private_json(companion_target, alternative_payload)
        companion_readback = json.loads(companion_target.read_text(encoding="utf-8"))
        if companion_readback != alternative_payload:
