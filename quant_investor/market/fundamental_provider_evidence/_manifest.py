...
def is_fundamental_provider_evidence_manifest(document: Mapping[str, Any]) -> bool:
    """Return whether a mapping selects the persisted full-rebuild contract."""

    return (
        isinstance(document, Mapping) and document.get("schema_version") == PROVIDER_MANIFEST_SCHEMA
    )
...
def _provider_relative(value: str) -> str:
    ...
    if not value.startswith(prefix):
        raise FundamentalProviderEvidenceError("provider evidence ref escaped its generation root")
...
def _assert_inventory_binding(
    ...
    *,
    ...
    if row is None or row["byte_sha256"] != byte_sha256:
        raise FundamentalProviderEvidenceError("provider fileset byte binding mismatch")


@provider_evidence_contract
def build_fundamental_provider_manifest(
    *,
    ...
    if receipt["plan_ref"] != content_ref(validated_plan, identity_field="plan_id"):
        raise FundamentalProviderEvidenceError("provider manifest plan closure mismatch")
    if receipt["status"] != "PASSED" or receipt["performance_gate_passed"] is not True:
        raise FundamentalProviderEvidenceError("provider manifest requires a passed reconciliation")
    ...
    if request_ref["available_at"] > created or coverage_ref["available_at"] > created:
        raise FundamentalProviderEvidenceError("provider manifest contains future files")
    ...
    if set(fingerprints) != set(SOURCE_TABLES):
        raise FundamentalProviderEvidenceError("provider raw fingerprint set is invalid")
    body = {
        **common_fields(timestamp_value=created),
        ...
        "schema_version": PROVIDER_MANIFEST_SCHEMA,
        ...
        "version": PROVIDER_MANIFEST_SCHEMA,
...
@provider_evidence_contract
def validate_fundamental_provider_manifest(
    ...
    **closure: Any,
    ...
    if value.get("version") != PROVIDER_MANIFEST_SCHEMA:
        raise FundamentalProviderEvidenceError("provider manifest v4 version mismatch")
    expected = build_fundamental_provider_manifest(
        **closure,
        created_at=value["timestamp"],
    )
    if value != expected:
        raise FundamentalProviderEvidenceError("provider manifest v4 replay mismatch")
    return value


__all__ = [
    "build_fundamental_provider_manifest",
