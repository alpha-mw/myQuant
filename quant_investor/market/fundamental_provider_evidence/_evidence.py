...
from ._endpoint_plan import validate_endpoint_execution_plan
from ._model import (
    LOGICAL_COVERAGE_SCHEMA,
    LOGICAL_STATUSES,
    PHYSICAL_REQUEST_RECEIPT_SCHEMA,
    PHYSICAL_STATUSES,
    RAW_TABLE_EVIDENCE_SCHEMA,
    SOURCE_TABLES,
...
def _codes(value: Any, *, label: str, maximum: int = 64) -> list[str]:
    ...
    for item in rows:
        if (
            ...
            raise FundamentalProviderEvidenceError(f"{label} contains an invalid code")
    ...
    if normalized != expected or len(normalized) != len(set(normalized)):
        raise FundamentalProviderEvidenceError(f"{label} must be ASCII-sorted unique")
    return normalized
...
def _partition(
    ...
    *,
    ...
    if len(rows) != 1:
        raise FundamentalProviderEvidenceError("physical partition is not in the sealed plan")
    return dict(rows[0])
...
def _validate_physical_status(
    *,
    ...
    if attempts < 1 or attempts > max_attempts:
        raise FundamentalProviderEvidenceError("physical request attempts exceed policy")
    if status == "AVAILABLE" and (
        ...
        raise FundamentalProviderEvidenceError("AVAILABLE physical request is not closed")
    if status == "EMPTY" and (
        reported or accepted or has_more or blockers or projection_sha is None
    ):
        raise FundamentalProviderEvidenceError("EMPTY physical request is not closed")
    if status == "INCOMPLETE" and (
        not blockers
        or any(code not in _INCOMPLETE_BLOCKERS for code in blockers)
        or projection_sha is None
    ):
        raise FundamentalProviderEvidenceError("INCOMPLETE physical blockers are invalid")
    if status in {"PROVIDER_ERROR", "SCHEMA_MISMATCH", "TRANSPORT_ERROR"} and (
        ...
        raise FundamentalProviderEvidenceError("failed physical request claims response data")
...
def _build_physical_request_receipt_validated(
    *,
    ...
    if table not in SOURCE_TABLES or status not in PHYSICAL_STATUSES:
        raise FundamentalProviderEvidenceError("physical table or status is invalid")
    ...
    if captured < validated_plan["created_at"]:
        raise FundamentalProviderEvidenceError("physical receipt predates its plan")
    ...
    if type(has_more) is not bool:
        raise FundamentalProviderEvidenceError("has_more must be boolean")
    ...
    body = {
        **common_fields(timestamp_value=captured),
        ...
        "table": table,
        "version": PHYSICAL_REQUEST_RECEIPT_SCHEMA,
    }
    return seal(body, identity_field="receipt_id")


@provider_evidence_contract
def build_provider_physical_request_receipt(
    *,
    plan: Mapping[str, Any],
    endpoint_plans: Mapping[str, Mapping[str, Any]],
    table: str,
...
@provider_evidence_contract
def validate_provider_physical_request_receipt(
...
def _validate_physical_request_receipt_validated(
    ...
    *,
    ...
    if value.get("version") != PHYSICAL_REQUEST_RECEIPT_SCHEMA:
        raise FundamentalProviderEvidenceError("physical request receipt version mismatch")
    ...
    if value != expected:
        raise FundamentalProviderEvidenceError("physical request receipt replay mismatch")
    return value
...
def _observed_interval(
    *,
    ...
    if (normalized_start is None) != (normalized_end is None):
        raise FundamentalProviderEvidenceError("logical observed interval is partial")
    if normalized_start is not None and (
        ...
        raise FundamentalProviderEvidenceError("logical observed interval is invalid")
...
def _build_logical_coverage_validated(
    *,
    ...
    if company_code not in validated_plan["symbols"] or table not in SOURCE_TABLES:
        raise FundamentalProviderEvidenceError("logical coverage subject is outside plan")
    if status not in LOGICAL_STATUSES:
        raise FundamentalProviderEvidenceError("logical coverage status is invalid")
    start = session_date(expected_start, label="expected_start")
    end = session_date(expected_end, label="expected_end")
    if start > end:
        raise FundamentalProviderEvidenceError("logical expected interval is reversed")
    ...
    if {row["partition_id"] for row in receipts} != expected_partition_ids:
        raise FundamentalProviderEvidenceError("logical physical partition keyset is incomplete")
    ...
    if (status == "COMPLETE") != complete:
        raise FundamentalProviderEvidenceError("logical coverage status does not match evidence")
    if count == 0 and observed_start_value is not None:
        raise FundamentalProviderEvidenceError("zero-row coverage claims an observed interval")
    if count > 0 and observed_start_value is None:
        raise FundamentalProviderEvidenceError("nonempty coverage lacks an observed interval")
    assessed = timestamp(assessed_at, label="assessed_at")
    if assessed < validated_plan["created_at"]:
        raise FundamentalProviderEvidenceError("logical coverage predates its plan")
    body = {
        **common_fields(timestamp_value=assessed),
        ...
        "status": status,
        "table": table,
        "version": LOGICAL_COVERAGE_SCHEMA,
    }
...
@provider_evidence_contract
def build_logical_symbol_table_coverage(
    *,
    plan: Mapping[str, Any],
    endpoint_plans: Mapping[str, Mapping[str, Any]],
...
def _validate_logical_coverage_validated(
    ...
    *,
    ...
    if value.get("version") != LOGICAL_COVERAGE_SCHEMA:
        raise FundamentalProviderEvidenceError("logical coverage version mismatch")
    ...
    if table not in SOURCE_TABLES:
        raise FundamentalProviderEvidenceError("logical coverage table is invalid")
    ...
    if value != expected:
        raise FundamentalProviderEvidenceError("logical coverage replay mismatch")
    return value


@provider_evidence_contract
def validate_logical_symbol_table_coverage(
    document: Mapping[str, Any],
    *,
    plan: Mapping[str, Any],
    endpoint_plans: Mapping[str, Mapping[str, Any]],
...
def _columns(value: Any) -> list[str]:
    ...
    for item in rows:
        if type(item) is not str or not item or not item.isascii():
            raise FundamentalProviderEvidenceError("column_order contains an invalid name")
        normalized.append(item)
    if not normalized or len(normalized) != len(set(normalized)):
        raise FundamentalProviderEvidenceError("column_order cardinality is invalid")
    return normalized


@provider_evidence_contract
def build_raw_table_evidence(
    *,
    plan: Mapping[str, Any],
    endpoint_plans: Mapping[str, Mapping[str, Any]],
    lane: str,
    ...
    if lane not in {"BASELINE", "VIP"} or table not in SOURCE_TABLES:
        raise FundamentalProviderEvidenceError("raw table lane or table is invalid")
    ...
    if (
        ...
        raise FundamentalProviderEvidenceError("raw table evidence contains future input")
    body = {
        **common_fields(timestamp_value=evidenced),
        ...
        "table": table,
        "version": RAW_TABLE_EVIDENCE_SCHEMA,
...
@provider_evidence_contract
def validate_raw_table_evidence(
    document: Mapping[str, Any],
    *,
    plan: Mapping[str, Any],
    endpoint_plans: Mapping[str, Mapping[str, Any]],
    ...
    if value.get("version") != RAW_TABLE_EVIDENCE_SCHEMA:
        raise FundamentalProviderEvidenceError("raw table evidence version mismatch")
    expected = build_raw_table_evidence(
        plan=plan,
        endpoint_plans=endpoint_plans,
        lane=value["lane"],
        table=value["table"],
    ...
    if value != expected:
        raise FundamentalProviderEvidenceError("raw table evidence replay mismatch")
    return value
