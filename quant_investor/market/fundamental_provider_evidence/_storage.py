...
from ._codec import canonical_bytes
from ._fileset import (
    REQUIRED_EVIDENCE_PATHS,
    validate_provider_evidence_fileset_manifest,
)
from ._model import FundamentalProviderEvidenceError, provider_evidence_contract
...
def _regular_bytes(path: Path) -> tuple[bytes, tuple[int, int]]:
    try:
        ...
        raise FundamentalProviderEvidenceError("provider evidence file is unavailable") from exc
    if (
        ...
        raise FundamentalProviderEvidenceError("provider evidence file is unsafe")
    try:
        descriptor = os.open(
            path,
        ...
        raise FundamentalProviderEvidenceError("provider evidence file open failed") from exc
    try:
    ...
    if (opened.st_dev, opened.st_ino, opened.st_size, opened.st_mtime_ns) != (
        ...
        raise FundamentalProviderEvidenceError("provider evidence file changed during read")
...
def _directory(path: Path) -> None:
    try:
        ...
        raise FundamentalProviderEvidenceError(
            "provider evidence directory is unavailable"
        ) from exc
    if (
        ...
        raise FundamentalProviderEvidenceError("provider evidence directory is unsafe")
...
def _actual_paths(root: Path) -> set[str]:
    ...
    if len(values) != len({value.casefold() for value in values}):
        raise FundamentalProviderEvidenceError("provider evidence has a casefold collision")
    return set(values)
...
def capture_provider_evidence_directory(root: str | Path) -> dict[str, bytes]:
    ...
    if not base.is_absolute() or ".." in base.parts or base.resolve(strict=True) != base:
        raise FundamentalProviderEvidenceError("provider evidence root must be absolute")
    _directory(base)
    required = {*REQUIRED_EVIDENCE_PATHS, "fileset_manifest.json"}
    if _actual_paths(base) != required:
        raise FundamentalProviderEvidenceError("provider evidence fileset is not exact")
    ...
    try:
        ...
        raise FundamentalProviderEvidenceError("fileset manifest is not JSON") from exc
    manifest = validate_provider_evidence_fileset_manifest(manifest_value)
    if manifest_bytes != canonical_bytes(manifest):
        raise FundamentalProviderEvidenceError("fileset manifest bytes are not canonical")
    ...
    for relative_path in REQUIRED_EVIDENCE_PATHS:
        ...
        if (
            ...
            raise FundamentalProviderEvidenceError("provider evidence byte identity mismatch")
        inodes.add(inode)
