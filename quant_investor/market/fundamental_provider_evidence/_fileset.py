"""Closed inventory for durable Fundamental provider evidence."""
...
from ._model import (
    PROVIDER_EVIDENCE_FILESET_SCHEMA,
    SOURCE_TABLES,
...
def _inventory(values: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalProviderEvidenceError("fileset inventory must be a sequence")
    ...
    for value in values:
        ...
        if (
            ...
            raise FundamentalProviderEvidenceError("fileset inventory row is invalid")
        rows.append(
            {
                "byte_sha256": sha256(
                    row.get("byte_sha256"),
    ...
    if (
        ...
        raise FundamentalProviderEvidenceError("fileset inventory is not exact and sorted")
    return rows
    ...
    *,
    ...
    body = {
        **common_fields(timestamp_value=created),
        ...
        "inventory": rows,
        "version": PROVIDER_EVIDENCE_FILESET_SCHEMA,
    }
...
def validate_provider_evidence_fileset_manifest(
    ...
    if value.get("version") != PROVIDER_EVIDENCE_FILESET_SCHEMA:
        raise FundamentalProviderEvidenceError("provider evidence fileset version mismatch")
    expected = build_provider_evidence_fileset_manifest(
        inventory=value["inventory"],
        created_at=value["created_at"],
    )
    if value != expected:
        raise FundamentalProviderEvidenceError("provider evidence fileset replay mismatch")
    return value
