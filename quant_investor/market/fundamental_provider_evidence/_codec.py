"""Private codec for replaying persisted Fundamental provider evidence.

This module exists only to verify the exact bytes and seals written by the
pre-cutover provider-evidence producer. It does not define stable active
Intelligence artifacts or publication authority.
"""
...
class FundamentalProviderEvidenceError(ValueError):
...
def require_exact_keys(value: Any, fields: set[str] | frozenset[str], *, label: str) -> dict:
    if type(value) is not dict or set(value) != set(fields):
        raise FundamentalProviderEvidenceError(f"{label} shape is invalid")
    return value


def timestamp(value: Any, *, label: str) -> str:
    if type(value) is not str or not value.endswith("Z"):
        raise FundamentalProviderEvidenceError(f"{label} must be a UTC second timestamp")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise FundamentalProviderEvidenceError(f"{label} is invalid") from exc
    if (
        ...
        raise FundamentalProviderEvidenceError(f"{label} must be canonical UTC seconds")
    return value


def session_date(value: Any, *, label: str) -> str:
    if type(value) is not str or re.fullmatch(r"\d{8}", value) is None:
        raise FundamentalProviderEvidenceError(f"{label} must be YYYYMMDD")
    try:
        parsed = datetime.strptime(value, "%Y%m%d")
    except ValueError as exc:
        raise FundamentalProviderEvidenceError(f"{label} is invalid") from exc
    if parsed.strftime("%Y%m%d") != value:
        raise FundamentalProviderEvidenceError(f"{label} is invalid")
    return value


def identifier(value: Any, *, label: str) -> str:
    if type(value) is not str or IDENTIFIER_RE.fullmatch(value) is None:
        raise FundamentalProviderEvidenceError(f"{label} must be a canonical identifier")
    return value


def code(value: Any, *, label: str) -> str:
    if type(value) is not str or CODE_RE.fullmatch(value) is None:
        raise FundamentalProviderEvidenceError(f"{label} must be an uppercase code")
    return value


def sha256(value: Any, *, label: str) -> str:
    if type(value) is not str or SHA256_RE.fullmatch(value) is None:
        raise FundamentalProviderEvidenceError(f"{label} must be lowercase SHA-256")
    return value
...
def decimal_value(
    ...
    *,
    ...
    if type(value) in {bool, float}:
        raise FundamentalProviderEvidenceError(f"{label} must not be binary float")
    try:
        parsed = Decimal(str(value))
    except (InvalidOperation, TypeError, ValueError) as exc:
        raise FundamentalProviderEvidenceError(f"{label} must be decimal") from exc
    if not parsed.is_finite():
        raise FundamentalProviderEvidenceError(f"{label} must be finite")
    if minimum is not None and parsed < minimum:
        raise FundamentalProviderEvidenceError(f"{label} is below its allowed domain")
    if maximum is not None and parsed > maximum:
        raise FundamentalProviderEvidenceError(f"{label} is above its allowed domain")
    return parsed
...
def _validate_text(value: str, *, label: str) -> None:
    if unicodedata.normalize("NFC", value) != value:
        raise FundamentalProviderEvidenceError(f"{label} must be Unicode NFC")
    try:
        encoded = value.encode("utf-8", errors="strict")
    except UnicodeEncodeError as exc:
        raise FundamentalProviderEvidenceError(f"{label} is not UTF-8") from exc
    if len(encoded) > 4000:
        raise FundamentalProviderEvidenceError(f"{label} exceeds 4000 UTF-8 bytes")
...
def _validate_dict(value: dict[str, Any], *, label: str, depth: int) -> None:
    for key, item in value.items():
        if type(key) is not str or not key or not key.isascii():
            raise FundamentalProviderEvidenceError(f"{label} keys must be nonempty ASCII")
...
def _validate_value(value: Any, *, label: str = "$", depth: int = 0) -> None:
    ...
    if type(value) is float:
        raise FundamentalProviderEvidenceError(f"{label} contains binary float")
    raise FundamentalProviderEvidenceError(f"{label} contains unsupported value")
...
def canonical_bytes(value: Any) -> bytes:
    ...
    if len(raw) > MAX_ARTIFACT_BYTES:
        raise FundamentalProviderEvidenceError("artifact exceeds 8 MiB")
    return raw
...
def seal(value: Mapping[str, Any], *, identity_field: str) -> dict[str, Any]:
    if type(value) is not dict or identity_field in value or "semantic_sha256" in value:
        raise FundamentalProviderEvidenceError("artifact must be an unsealed object")
    result = dict(value)
...
def validate_seal(value: Mapping[str, Any], *, identity_field: str) -> dict[str, Any]:
    if type(value) is not dict:
        raise FundamentalProviderEvidenceError("artifact must be an object")
    result = dict(value)
    if sha256(result.get(identity_field), label=identity_field) != _identity(
        result, identity_field=identity_field
    ):
        raise FundamentalProviderEvidenceError(f"{identity_field} mismatch")
    if sha256(result.get("semantic_sha256"), label="semantic_sha256") != _semantic(result):
        raise FundamentalProviderEvidenceError("semantic_sha256 mismatch")
    canonical_bytes(result)
    return result
...
def _safe_path(value: Any, *, label: str) -> str:
    if (
        ...
        raise FundamentalProviderEvidenceError(f"{label} must be a safe relative path")
    return value
...
def exact_ref(value: Mapping[str, Any], *, label: str) -> dict[str, str]:
    ...
    if not artifact_id or not artifact_version:
        raise FundamentalProviderEvidenceError(f"{label} identity fields are required")
    ...
    if cutoff > available_at:
        raise FundamentalProviderEvidenceError(f"{label} cutoff exceeds availability")
    return {
...
def require_no_future(*, available_at: str, as_of: str, label: str) -> None:
    if timestamp(available_at, label=f"{label}.available_at") > timestamp(as_of, label="as_of"):
        raise FundamentalProviderEvidenceError(f"{label} contains future evidence")
