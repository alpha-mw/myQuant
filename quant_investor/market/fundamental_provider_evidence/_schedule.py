...
def _symbols(values: Sequence[str]) -> list[str]:
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalProviderEvidenceError("symbols must be a sequence")
    rows = list(values)
    if not rows or len(rows) > 10_000:
        raise FundamentalProviderEvidenceError("symbol scope cardinality is invalid")
    normalized: list[str] = []
    for value in rows:
        if type(value) is not str or not value or not value.isascii():
            raise FundamentalProviderEvidenceError("symbol scope contains an invalid value")
    ...
    if normalized != expected or len(normalized) != len(set(normalized)):
        raise FundamentalProviderEvidenceError("symbol scope must be ASCII-sorted unique")
    return normalized


def _open_sessions(values: Sequence[str], *, start: str, end: str) -> list[str]:
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalProviderEvidenceError("daily_open_sessions must be a sequence")
    ...
    if (
        ...
        raise FundamentalProviderEvidenceError("daily open-session closure is invalid")
    return rows
...
def _validated_endpoint_plans(
    ...
    if type(value) is not dict or set(value) != set(SOURCE_ENDPOINTS):
        raise FundamentalProviderEvidenceError("endpoint plan set is invalid")
    ...
    for table in sorted(SOURCE_ENDPOINTS):
        ...
        if (
            ...
            raise FundamentalProviderEvidenceError("endpoint plan authority mismatch")
        plans[table] = plan
...
def _baseline_empty_keyset(
    ...
    *,
    ...
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalProviderEvidenceError("baseline_empty_partition_keyset must be a sequence")
    rows = list(values)
    ...
    if (
        ...
        raise FundamentalProviderEvidenceError("baseline empty partition keyset is invalid")
    return rows
...
def build_fundamental_request_plan(
    *,
    ...
    """Build the exact inclusive schedule required by Fundamental VIP v4."""
    ...
    if cutoff > created or cutoff[:10].replace("-", "") > target:
        raise FundamentalProviderEvidenceError("PIT cutoff is future-dated")
    ...
    if type(max_attempts_per_partition) is not int or not (1 <= max_attempts_per_partition <= 64):
        raise FundamentalProviderEvidenceError("max_attempts_per_partition is invalid")
    if type(baseline_network_attempts) is not int or baseline_network_attempts < 1:
        raise FundamentalProviderEvidenceError("baseline network attempts are invalid")
    plans, endpoint_refs = _validated_endpoint_plans(endpoint_plans)
    if any(plan["created_at"] > created for plan in plans.values()):
        raise FundamentalProviderEvidenceError("request plan contains future endpoint policy")
    ...
    if (
        ...
        raise FundamentalProviderEvidenceError("market closure contains future evidence")
...
def validate_fundamental_request_plan(
    ...
    *,
    ...
    require_exact_keys(value, _FIELDS, label="Fundamental request plan v4")
    if value.get("version") != REQUEST_PLAN_SCHEMA:
        raise FundamentalProviderEvidenceError("Fundamental request plan version mismatch")
    ...
    if value != expected:
        raise FundamentalProviderEvidenceError("Fundamental request plan replay mismatch")
    return value
...
def build_fundamental_execution_closure(
    *,
    ...
    if created < validated["created_at"]:
        raise FundamentalProviderEvidenceError("execution closure predates request plan")
    return seal(
        {
            **common_fields(timestamp_value=created),
            "created_at": created,
            ...
            "version": EXECUTION_CLOSURE_SCHEMA,
        },
...
def validate_fundamental_execution_closure(
    ...
    require_exact_keys(value, _EXECUTION_FIELDS, label="Fundamental execution closure v4")
    if value.get("version") != EXECUTION_CLOSURE_SCHEMA:
        raise FundamentalProviderEvidenceError("execution closure version mismatch")
    ...
    if value != expected:
        raise FundamentalProviderEvidenceError("execution closure replay mismatch")
    return value
