        ...
        canonical_bytes(
            {
                ...
                "transport": "OfficialTushareHttpsClient",
            }
        )
    ).hexdigest()
...
def _validate_plan_identity(
    *,
    ...
    if type(strict_decimal_decode) is not bool or strict_decimal_decode is not True:
        raise TushareContractError("endpoint plans require strict decimal decode")
...
def validate_endpoint_execution_plan(document: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if value.get("kind") != EXECUTION_PLAN_KIND:
        raise TushareContractError("endpoint execution plan kind mismatch")
...
def validate_tushare_endpoint_policy(document: Mapping[str, Any]) -> dict[str, Any]:
    ...
    require_exact_keys(
        value,
        _COMMON_FIELDS | {"created_at", "endpoint_plans", "policy_id"},
        label="Tushare endpoint policy",
    )
    if value.get("kind") != ENDPOINT_POLICY_KIND:
        raise TushareContractError("Tushare endpoint policy kind mismatch")
...
def validate_tushare_request_receipt(
    ...
    *,
    ...
    if value.get("kind") != REQUEST_RECEIPT_KIND:
        raise TushareContractError("Tushare request receipt kind mismatch")
...
def build_tushare_capability_receipt(
    *,
    ...
    if (
        ...
        and normalized_request_ref["kind"] != REQUEST_RECEIPT_KIND
    ):
        raise TushareContractError("request_ref kind mismatch")
...
def validate_tushare_capability_receipt(
    ...
    *,
    ...
    if value.get("kind") != CAPABILITY_RECEIPT_KIND:
        raise TushareContractError("Tushare capability receipt kind mismatch")
...
def build_tushare_execution_receipt(
    *,
    ...
    if any(item["kind"] != REQUEST_RECEIPT_KIND for item in refs):
        raise TushareContractError("request_refs contain an unsupported kind")
...
def validate_tushare_execution_receipt(
    ...
    *,
    ...
    if value.get("kind") != EXECUTION_RECEIPT_KIND:
        raise TushareContractError("Tushare execution receipt kind mismatch")
    ...
    *,
    ...
    body = {
        **common_fields(timestamp_value=captured),
        ...
        "kind": SCHEMA_DIAGNOSTIC_RECEIPT_KIND,
    }
...
def validate_tushare_schema_diagnostic_receipt(
    ...
    *,
    ...
    if value.get("kind") != SCHEMA_DIAGNOSTIC_RECEIPT_KIND:
        raise TushareContractError("schema diagnostic receipt kind mismatch")
...
def response_projection_sha256(rows: Sequence[Sequence[Any]]) -> str:
    """Hash exact row order without embedding provider text in an artifact."""
