"""Bounded execution of a stable Fundamental partition plan."""
...
from .models import FundamentalAcquisitionError, SOURCE_TABLES
...
def _private_directory(path: Path) -> None:
    ...
    if (
        ...
        raise FundamentalAcquisitionError("VIP checkpoint directory is unsafe")
...
def _checkpoint_root(
    ...
    *,
    ...
    if not root.is_absolute() or ".." in root.parts or root.is_symlink():
        raise FundamentalAcquisitionError("VIP checkpoint root must be absolute")
    ...
    try:
        ...
        raise FundamentalAcquisitionError("VIP checkpoint creation failed") from exc
    ...
    if closure_path.exists():
        if _read_private_file(closure_path) != execution_bytes:
            raise FundamentalAcquisitionError("VIP checkpoint closure changed")
    else:
...
def _validate_checkpoint_names(root: Path, records: Path) -> None:
    ...
    if root_names != ["execution_closure.json", "partition_records"]:
        raise FundamentalAcquisitionError("VIP checkpoint root contains unknown files")
    ...
    if len(committed) != len({name.casefold() for name in committed}) or any(
        ...
        raise FundamentalAcquisitionError("VIP checkpoint record names are invalid")
...
def _read_private_file(path: Path) -> bytes:
    ...
    if (
        ...
        raise FundamentalAcquisitionError("VIP checkpoint file is unsafe")
    ...
    if (opened.st_ino, opened.st_dev, opened.st_size, opened.st_mtime_ns) != (
        ...
        raise FundamentalAcquisitionError("VIP checkpoint file changed during read")
...
def _write_exact(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise FundamentalAcquisitionError("VIP checkpoint write stalled")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
    if _read_private_file(path) != payload:
        raise FundamentalAcquisitionError("VIP checkpoint readback failed")
...
def _atomic_record(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        os.close(descriptor)
    if path.exists():
        raise FundamentalAcquisitionError("VIP checkpoint partition already exists")
    ...
    try:
        ...
        os.close(directory)
    if _read_private_file(path) != payload:
        raise FundamentalAcquisitionError("VIP checkpoint partition readback failed")
    ...
    if type(value) is str:
        ...
        if len(encoded) > 4000:
            if unicodedata.normalize("NFC", value) != value:
                raise FundamentalAcquisitionError("VIP checkpoint text is not Unicode NFC")
        ...
        return ["TEXT", value]
    raise FundamentalAcquisitionError("VIP checkpoint row scalar is invalid")


def _restore_text_chunks(value: Any) -> str:
    if type(value) is not list or not value:
        raise FundamentalAcquisitionError("VIP checkpoint text chunks are invalid")
    chunks: list[bytes] = []
    for chunk in value:
        if type(chunk) is not str or not chunk.isascii() or len(chunk) > 4000:
            raise FundamentalAcquisitionError("VIP checkpoint text chunk is invalid")
        try:
            decoded = base64.b64decode(chunk, validate=True)
        except (binascii.Error, ValueError, TypeError) as exc:
            raise FundamentalAcquisitionError("VIP checkpoint text chunk is invalid") from exc
        if base64.b64encode(decoded).decode("ascii") != chunk:
            raise FundamentalAcquisitionError("VIP checkpoint text chunk is not canonical")
        chunks.append(decoded)
    encoded = b"".join(chunks)
    try:
        restored = encoded.decode("utf-8", errors="strict")
    except UnicodeDecodeError as exc:
        raise FundamentalAcquisitionError("VIP checkpoint text is not UTF-8") from exc
    if len(encoded) <= 4000 or unicodedata.normalize("NFC", restored) != restored:
        raise FundamentalAcquisitionError("VIP checkpoint chunked text is invalid")
    return restored


def _restore_scalar(value: Any) -> Any:
    if not isinstance(value, list) or len(value) != 2:
        raise FundamentalAcquisitionError("VIP checkpoint scalar shape is invalid")
    kind, scalar = value
    ...
    raise FundamentalAcquisitionError("VIP checkpoint scalar is invalid")
...
def _read_record(
    ...
    *,
    ...
    try:
        ...
        raise FundamentalAcquisitionError("VIP checkpoint record is invalid") from exc
    if canonical_bytes(value) != raw or set(value) != {"receipt", "rows"}:
        raise FundamentalAcquisitionError("VIP checkpoint record is not canonical")
    receipt = _validate_validated(
        value["receipt"],
        validated_plan=plan,
    ...
    if (projection is None and rows) or (
        ...
        raise FundamentalAcquisitionError("VIP checkpoint row closure mismatch")
    return receipt, rows
...
def _params(
    ...
    *,
    ...
    if endpoint_plan["partition_dimensions"] != [dimension]:
        raise FundamentalAcquisitionError("endpoint partition dimension mismatch")
    fixed = dict(endpoint_plan["fixed_params"])
    if dimension in fixed:
        raise FundamentalAcquisitionError("partition dimension must not be fixed")
...
def _assert_endpoint_keysets(
    ...
    for table in SOURCE_TABLES:
        ...
        if (
            ...
            raise FundamentalAcquisitionError("endpoint partition keyset is incomplete")
...
def acquire_fundamental_partitions(
    *,
    execution_closure: Mapping[str, Any],
    client: TushareRequestClient,
    captured_at: str,
    ...
    execution = validate_fundamental_execution_closure(execution_closure)
    ...
    for partition in plan["partition_rows"]:
        ...
        if record_path is not None and record_path.exists():
            ...
            if (
                ...
                raise FundamentalAcquisitionError("VIP checkpoint partition identity mismatch")
        else:
        ...
        if attempts > plan["planned_max_network_attempts"]:
            raise FundamentalAcquisitionError("VIP acquisition exceeded attempt bound")
    if len(receipts) != plan["planned_terminal_request_count"]:
        raise FundamentalAcquisitionError("VIP acquisition terminal keyset is incomplete")
...
__all__ = ["acquire_fundamental_partitions"]
