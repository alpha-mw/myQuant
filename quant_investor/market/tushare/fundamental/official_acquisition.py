"""Execute the stable official-parameter Fundamental shadow plan."""
...
def _codes(values: Sequence[str]) -> list[str]:
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise FundamentalAcquisitionError("official request blockers must be a sequence")
    rows = list(values)
    if any(type(value) is not str or value not in _BLOCKERS for value in rows):
        raise FundamentalAcquisitionError("official request blocker is invalid")
    expected = sorted(rows, key=lambda value: value.encode("ascii"))
    if rows != expected or len(rows) != len(set(rows)):
        raise FundamentalAcquisitionError("official request blockers are not canonical")
    return rows
...
def _validate_receipt_status(
    *,
    ...
    if status not in _STATUSES:
        raise FundamentalAcquisitionError("official request status is invalid")
    if (
        ...
        raise FundamentalAcquisitionError("official request counts are invalid")
    if status == "AVAILABLE" and (
        ...
        raise FundamentalAcquisitionError("AVAILABLE official request is not closed")
    if status == "EMPTY" and (
        reported_count or accepted_count or has_more or blockers or projection_sha256 is None
    ):
        raise FundamentalAcquisitionError("EMPTY official request is not closed")
    if status == "INCOMPLETE" and not blockers:
        raise FundamentalAcquisitionError("INCOMPLETE official request lacks a blocker")
    if status in {"PROVIDER_ERROR", "SCHEMA_MISMATCH", "TRANSPORT_ERROR"} and (
        ...
        raise FundamentalAcquisitionError("failed official request claims response data")
...
def _build_receipt(
    *,
    ...
    if captured < plan["created_at"]:
        raise FundamentalAcquisitionError("official request receipt predates its plan")
    ...
    body = {
        **common_fields(timestamp_value=captured),
        ...
        "status": status,
        "table": request["table"],
        "kind": OFFICIAL_PARTITION_RECEIPT_KIND,
    }
...
def _validate_receipt_validated(
    ...
    *,
    ...
    if receipt.get("kind") != OFFICIAL_PARTITION_RECEIPT_KIND:
        raise FundamentalAcquisitionError("official request receipt kind mismatch")
    request = requests_by_key.get(receipt["request_key"])
    if request is None:
        raise FundamentalAcquisitionError("official request is not in the sealed plan")
    ...
    if receipt != expected:
        raise FundamentalAcquisitionError("official request receipt replay mismatch")
    return receipt
...
def _checkpoint(
    ...
    *,
    ...
    if not root.is_absolute() or ".." in root.parts or root.is_symlink():
        raise FundamentalAcquisitionError("official checkpoint root must be absolute")
    try:
        os.mkdir(root, 0o700)
    ...
    if closure.exists():
        if _read_private_file(closure) != payload:
            raise FundamentalAcquisitionError("official checkpoint execution bundle changed")
    else:
...
def _checkpoint_names(root: Path) -> None:
    if sorted(item.name for item in os.scandir(root)) != [
        ...
        raise FundamentalAcquisitionError("official checkpoint contains unknown files")
    records = root / "partition_records"
    ...
    if len(committed) != len({name.casefold() for name in committed}) or any(
        ...
        raise FundamentalAcquisitionError("official checkpoint record names are invalid")
...
def _read_record(
    ...
    *,
    ...
    try:
        ...
        raise FundamentalAcquisitionError("official checkpoint record is invalid") from exc
    if canonical_bytes(value) != raw or set(value) != {"receipt", "rows"}:
        raise FundamentalAcquisitionError("official checkpoint record is not canonical")
    ...
    if (projection is None and rows) or (
        ...
        raise FundamentalAcquisitionError("official checkpoint row closure mismatch")
    return receipt, rows
...
def _baseline_fingerprints(
    ...
    *,
    ...
    if type(values) is not dict or set(values) != set(SOURCE_TABLES):
        raise FundamentalAcquisitionError("baseline fingerprint set is invalid")
    if type(baseline_tables) is not dict or set(baseline_tables) != set(SOURCE_TABLES):
        raise FundamentalAcquisitionError("baseline table set is invalid")
    result: dict[str, str] = {}
    for table in SOURCE_TABLES:
        ...
        if not isinstance(frame, pd.DataFrame) or frame_fingerprint(frame) != expected:
            raise FundamentalAcquisitionError("baseline frame fingerprint mismatch")
...
def _accepted_symbol_rows(
    ...
    *,
    ...
    try:
        ...
        raise FundamentalAcquisitionError("official accepted-raw PIT projection failed") from exc
    if malformed_reason:
        raise FundamentalAcquisitionError("official accepted-raw PIT projection is malformed")
    if table == "forecast":
...
def _accepted_raw_projection(
    ...
    *,
    ...
    """Apply the frozen per-symbol accepted-raw boundary to bulk responses."""
    ...
    for table in SOURCE_TABLES:
        ...
        if "ts_code" not in frame.columns:
            raise FundamentalAcquisitionError("official raw table is missing target-scope identity")
...
def _physical_endpoint_plans(
    ...
    *,
    ...
    if official_plan["kind"] != OFFICIAL_PARTITION_PLAN_KIND:
        return plans
    ...
    if physical_columns != ["report_type", "comp_type"]:
        raise FundamentalAcquisitionError("official physical projection is invalid")
    for table in ("balancesheet", "cashflow", "income"):
        ...
        if any(column in fields for column in physical_columns):
            raise FundamentalAcquisitionError("official physical projection duplicates a field")
        plans[table]["expected_fields"] = fields + list(physical_columns)
    return plans


@fundamental_contract
def acquire_official_fundamental_partitions(
    *,
    official_plan: Mapping[str, Any],
    source_execution_closure: Mapping[str, Any],
    probe_observations: Sequence[Mapping[str, Any]],
    ...
    captured = timestamp(captured_at, label="captured_at")
    if captured < plan["created_at"]:
        raise FundamentalAcquisitionError("official acquisition predates its plan")
    ...
    bundle = {
        ...
        "comparison_policy_sha256": hashlib.sha256(
            canonical_provider_json_bytes(policy)
        ).hexdigest(),
        "official_plan_ref": content_ref(
            plan,
            identity_field="partition_plan_id",
        ),
    ...
    transport_calls = 0
    plan_ref = content_ref(plan, identity_field="partition_plan_id")
    request_rows = replay_official_partition_requests(
        plan,
        source_execution_closure=source,
    )
    ...
    for request in request_rows:
        ...
        if record_path is not None and record_path.exists():
            ...
            if receipt["request_key"] != request["request_key"]:
                raise FundamentalAcquisitionError("official checkpoint request identity mismatch")
        else:
            receipt, rows = _fetch(
                client=client,
                plan=plan,
