...
class CalibrationCurve:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "CalibrationCurve":
        data = dict(payload)
        return cls(
            schema_version=str(data.get("schema_version", CALIBRATION_V2_SCHEMA_VERSION)),
            key=CalibrationCurveKey.from_dict(dict(data.get("key", {}) or {})),
            bucket_count=int(data.get("bucket_count", 10) or 10),
            prior_strength=float(data.get("prior_strength", 20.0) or 20.0),
            total_examples=int(data.get("total_examples", 0) or 0),
            positive_examples=int(data.get("positive_examples", 0) or 0),
            base_rate=_float_or_none(data.get("base_rate")),
            buckets=[
                CalibrationBucket.from_dict(bucket)
                for bucket in list(data.get("buckets", []) or [])
...
class CalibrationModelV2:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "CalibrationModelV2":
        ...
        return cls(
            ...
            bucket_count=int(data.get("bucket_count", 10) or 10),
            prior_strength=float(data.get("prior_strength", 20.0) or 20.0),
            min_examples_per_curve=int(data.get("min_examples_per_curve", 30) or 30),
            curves=[
                CalibrationCurve.from_dict(curve)
                for curve in list(data.get("curves", []) or [])
...
class CalibrationV2Store:
    ...
    def save_model(self, model: CalibrationModelV2) -> Path:
        return self._write_json(self.model_path, model.to_dict())
