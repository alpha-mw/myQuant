"""Strict canonical JSON and semantic SHA-256 helpers for V17 v4."""
...
def validate_json_limits(value: Any, *, label: str = "$") -> None:
    ...
    def walk(current: Any, *, depth: int, path: str) -> None:
        ...
        if type(current) is dict:
            ...
            return
        raise CanonicalContractError(f"unsupported JSON type at {path}")
...
def strict_json_loads(
    ...
    *,
    ...
    if type(raw) is not bytes or len(raw) > bound or raw.startswith(b"\xef\xbb\xbf"):
        raise CanonicalContractError(f"{label} is not bounded canonical UTF-8 JSON")
    try:
        value = json.loads(
...
def canonical_resource_bytes(value: Any, *, max_bytes: int | None = None) -> bytes:
    raw = canonical_bytes(value, max_bytes=max_bytes)
    bound = LIMITS["general_json_bytes"] if max_bytes is None else max_bytes
    if len(raw) >= bound:
    ...
    *,
    ...
    if raw != canonical_resource_bytes(value, max_bytes=max_bytes):
        raise CanonicalContractError(
            f"{label} is not canonical compact JSON plus newline"
        )
    return value
...
def seal_semantic(value: Any) -> dict[str, Any]:
    if type(value) is not dict or SEMANTIC_SHA_FIELD in value:
        raise CanonicalContractError("sealer requires an unsealed object")
    result = dict(value)
    result[SEMANTIC_SHA_FIELD] = semantic_sha256(result)
    return result


def validate_semantic_sha(value: Any) -> dict[str, Any]:
    if type(value) is not dict:
        raise CanonicalContractError("semantic payload must be an object")
    try:
        declared = require_sha256(value.get(SEMANTIC_SHA_FIELD))
    except IdentityContractError as exc:
        raise CanonicalContractError(str(exc)) from exc
    if declared != semantic_sha256(value):
