...
def _extract_recommendation_timestamp(recommendations: dict[str, dict[str, Any]]) -> str | None:
    timestamps = [
        str(item.get("source_batch_timestamp", "")).strip()
...
def run_tracker(args: argparse.Namespace) -> dict[str, Any]:
    ...
    target_latest_trade_date = downloader.detect_latest_available_trade_date()
    completeness_before = downloader.build_completeness_report(
        universe=universe,
        required_latest_trade_date=target_latest_trade_date,
    )
    ...
    recommendation_timestamp = _extract_recommendation_timestamp(recommendations)
    ...
    report_lines.extend(
        [
            ...
            f"- 外部探测最新交易日：`{target_latest_trade_date or '未知'}`",
            f"- 本地主导最新交易日：`{completeness_before.get('dominant_latest_trade_date') or '未知'}`",
    ...
    for category in ["large_cap", "mid_cap", "small_cap"]:
        ...
        blocking_stale = payload.get("blocking_stale_symbols", [])
        if blocking_stale:
            report_lines.append(
                "- "
                + category
                + " 阻塞旧档：`"
                + ", ".join(
                    f"{item['symbol']}@{item['latest_local_date']}" for item in blocking_stale
                )
                + "`"
            )
        blocking_missing = payload.get("blocking_missing_symbols", [])
        if blocking_missing:
            report_lines.append(
                "- "
                + category
    ...
    report_lines.extend(
        [
            ...
            "## 3A. 模型推荐快照状态",
            "",
            f"- 最近推荐批次时间：`{recommendation_timestamp or '未知'}`",
            (
                f"- 推荐快照与最新交易日一致性：`{'已对齐' if recommendation_timestamp and recommendation_timestamp[:8] >= latest_trade_date.replace('-', '') else '存在滞后'}`"
            ),
            "",
            "## 4. 美股整体风格、广度与指数结构",
            "",
