...
LOCAL_TOKENIZER_DIR_BY_MODEL = {
    "kronos-mini": "tokenizer-2k",
    "kronos-small": "tokenizer-base",
...
class KronosIntegrator:
    ...
    使用方式：
    ----------
    integrator = KronosIntegrator(model_name="kronos-base")
    ...
    def __init__(
        ...
        self._model_info = KRONOS_MODELS.get(model_name, KRONOS_MODELS["kronos-base"])
    ...
    @property
    def native_model_loaded(self) -> bool:
        """返回是否已加载 vendored 原生模型。"""
        return self._model_loaded and self._predictor is not None

    def _resolve_runtime_device(self) -> str:
        """解析 Kronos 推理设备。

        默认避免在 fork 子进程内探测 MPS，防止 macOS + PyTorch 组合下直接 abort。
        """
    ...
    def _try_load_model(self) -> bool:
        ...
        try:
            ...
            _logger.warning(
                "Kronos 当前已使用 myQuant 内置 vendored 模型代码；"
                "若本地权重不可用，将自动回退到统计替代模式。"
            )
            return False
    ...
    def _try_load_kronos_native(self) -> bool:
        ...
        if self._model_loaded:
            return True
        try:
            from quant_investor._vendor.kronos_model import Kronos, KronosPredictor, KronosTokenizer

            model_source = self._resolve_model_source()
            tokenizer_source = self._resolve_tokenizer_source()
            if model_source is None or tokenizer_source is None:
                self._runtime_mode = "statistical_fallback"
                _logger.info("Kronos 未配置 myQuant 内本地权重目录，使用统计替代模式")
                return False
            max_context = self._model_info["context"]
            tokenizer = self._safe_from_pretrained(KronosTokenizer, tokenizer_source)
            model = self._safe_from_pretrained(Kronos, model_source)
    ...
    def _resolve_model_source(self) -> str | None:
        candidate = self._resolve_local_asset_dir(self.model_name)
        if candidate is not None:
            return candidate
        if self.allow_remote_download:
            return str(self._model_info["hub_id"])
        return None

    def _resolve_tokenizer_source(self) -> str | None:
        tokenizer_dir = LOCAL_TOKENIZER_DIR_BY_MODEL.get(self.model_name, "tokenizer-base")
        candidate = self._resolve_local_asset_dir(tokenizer_dir)
        if candidate is not None:
            return candidate
        if self.allow_remote_download:
            return DEFAULT_TOKENIZER_HUB
        return None

    def _resolve_local_asset_dir(self, asset_name: str) -> str | None:
        if not self.model_path:
            return None

        root = Path(self.model_path).expanduser()
        search_dirs = [root]
        if root.name == asset_name:
            search_dirs.append(root)
        else:
            search_dirs.append(root / asset_name)
            search_dirs.append(root.parent / asset_name)
    ...
    def _safe_from_pretrained(self, klass, source: str):
        kwargs = {}
        is_local_path = Path(str(source)).expanduser().exists()
        if not is_local_path:
            kwargs["local_files_only"] = not self.allow_remote_download
        previous_offline = os.environ.get("HF_HUB_OFFLINE")
        previous_transformers_offline = os.environ.get("TRANSFORMERS_OFFLINE")
        if not self.allow_remote_download and not is_local_path:
            os.environ["HF_HUB_OFFLINE"] = "1"
            os.environ["TRANSFORMERS_OFFLINE"] = "1"
        try:
            return klass.from_pretrained(source, **kwargs)
        except TypeError:
    ...
    @staticmethod
    def prepare_ohlcv(df: pd.DataFrame, max_context: int = 512) -> pd.DataFrame:
        """
