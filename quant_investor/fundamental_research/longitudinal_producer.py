...
def _load_analysis_manifest(path: str | Path) -> tuple[dict[str, Any], Path, str]:
    ...
    for component in (absolute, *absolute.parents):
        if component.is_symlink():
            raise LongitudinalProducerError(
                "analysis manifest path must not contain symlinks"
            )
    target = absolute.resolve(strict=True)
    _repo_relative(target)
    if not target.is_file() or target.stat().st_mode & 0o777 != PRIVATE_MODE:
        raise LongitudinalProducerError(
...
def _require_nav_attribution_basis(
    weights_by_variant: dict[str, dict[str, float]],
) -> None:
    if not any(
        abs(float(weight)) > 1e-12
        for weights in weights_by_variant.values()
        for weight in weights.values()
    ):
        raise LongitudinalProducerError(
            "NAV attribution requires at least one nonzero canonical target weight"
        )
...
def produce_nav_attribution_observation(
    *,
    ...
    _require_nav_attribution_basis(weights_by_variant)
