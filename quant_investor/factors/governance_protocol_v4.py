...
def protocol_policy() -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_admission": {
            ...
            "high_correlation_dedup_required": True,
            "initial_weight": 0.0,
    ...
    *,
    ...
    for raw in month_end_rankic_dates:
        try:
            ...
            continue
        month_ends.setdefault(observed.isoformat()[:7], observed.isoformat())
    ...
    for raw in forward_cohorts:
        ...
        if cohort_id and horizon == 30 and end > start:
            candidates.append((start, end, cohort_id))
...
def assess_candidate_admission_v4(candidate: Mapping[str, Any]) -> dict[str, Any]:
    """Assess a zero-weight v4 candidate proposal without writing a registry."""
    ...
    if candidate.get("duplicate_primitive") is not False:
        blockers.append("candidate_duplicate_primitive_not_false")
    if candidate.get("high_correlation_dedup_passed") is not True:
    ...
    q_value = candidate.get("bh_q_value")
    if (
        isinstance(q_value, bool)
        or not isinstance(q_value, (int, float))
        or not math.isfinite(float(q_value))
        or not 0.0 <= float(q_value) <= FDR_Q
    ):
        blockers.append("candidate_bh_q_value_above_0.10_or_invalid")
    if candidate.get("fdr_method") != "benjamini_hochberg_by_family":
        blockers.append("candidate_bh_method_mismatch")
    ...
    if not isinstance(evidence, Mapping):
        ...
        if evidence.get("schema_version") != FACTOR_EVIDENCE_SCHEMA_VERSION:
            blockers.append("candidate_v4_evidence_schema_mismatch")
        if evidence.get("status") != "verified":
            blockers.append("candidate_v4_evidence_not_verified")
        if not _is_sha256(evidence.get("replay_semantic_sha256")):
            blockers.append("candidate_v4_replay_sha_invalid")
    blockers = list(dict.fromkeys(blockers))
    return {
