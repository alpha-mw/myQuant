...
def _guard_record(tmp_path: Path) -> Path:
    run_dir = tmp_path / "record"
    run_dir.mkdir()
    (run_dir / "manifest.json").write_text(
        json.dumps({"timestamp": "run-guard"}),
        encoding="utf-8",
    )
    (run_dir / "market_snapshot.json").write_text(
        json.dumps({"analysis_trade_date": "20260618"}),
        encoding="utf-8",
    )
    (run_dir / "analysis_report.md").write_text("base report\n", encoding="utf-8")
    (run_dir / "holdings_review.csv").write_text(
        "\n".join(
            [
                "symbol,name,current_price,stage_stop_price",
                "000001.SZ,平安银行,10.00,9.00",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def _guard_snapshot(phase: str = "distribution") -> dict[str, object]:
    return {
        "theme_rotation": {
            "status": "success",
            "symbol_primary_theme": {"000001.SZ": "industry::bank"},
            "symbol_phase": {"000001.SZ": phase},
            "symbol_risk_flags": {"000001.SZ": []},
            "theme_scores": {
                "industry::bank": {
                    "theme_id": "industry::bank",
                    "theme_name": "银行",
                    "phase": phase,
                }
            },
...
def test_theme_holding_guard_disabled_leaves_review_report_unchanged(monkeypatch, tmp_path):
    run_dir = _guard_record(tmp_path)
    before = (run_dir / "analysis_report.md").read_text(encoding="utf-8")
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_ENABLED", False, raising=False)

    daily_review._apply_theme_holding_guard_to_record(
        run_dir,
        theme_snapshot={"status": "success"},
        theme_rotation=_guard_snapshot()["theme_rotation"],
        snapshot_payload=_guard_snapshot(),
    )

    assert (run_dir / "analysis_report.md").read_text(encoding="utf-8") == before


def test_theme_holding_guard_enabled_appends_tightened_display_stop(monkeypatch, tmp_path):
    run_dir = _guard_record(tmp_path)
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_ENABLED", True, raising=False)
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_TIGHTEN_RATIO", 0.5, raising=False)

    payload = _guard_snapshot()
    daily_review._apply_theme_holding_guard_to_record(
        run_dir,
        theme_snapshot={"status": "success"},
    ...
    assert "## 主题状态守卫" in report
    assert "`000001.SZ` 平安银行" in report
    assert "guard=tighten" in report
    assert "展示止损 9.50" in report
    assert "主题转弱（distribution），止损缓冲收紧" in report
    assert manifest["formal_diagnostics"]["theme_holding_guard"]["status"] == "success"


def test_theme_holding_guard_custom_ratio_changes_display_stop(monkeypatch, tmp_path):
    run_dir = _guard_record(tmp_path)
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_ENABLED", True, raising=False)
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_TIGHTEN_RATIO", 0.25, raising=False)
    ...
    report = (run_dir / "analysis_report.md").read_text(encoding="utf-8")
    assert "展示止损 9.75" in report


def test_theme_holding_guard_snapshot_unavailable_reports_fail_open(monkeypatch, tmp_path):
    run_dir = _guard_record(tmp_path)
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_ENABLED", True, raising=False)

    daily_review._apply_theme_holding_guard_to_record(
        run_dir,
        theme_snapshot={"status": "not_persisted_as_standalone_file"},
        theme_rotation={},
        snapshot_payload={},
    )
    ...
    report = (run_dir / "analysis_report.md").read_text(encoding="utf-8")
    assert "## 主题状态守卫" in report
    assert "theme snapshot unavailable" in report


def test_theme_holding_guard_attach_overlay_integration(monkeypatch, tmp_path):
    run_dir = _guard_record(tmp_path)
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_ENABLED", True, raising=False)
    monkeypatch.setattr(daily_review.config, "THEME_HOLDING_GUARD_TIGHTEN_RATIO", 0.5, raising=False)
    monkeypatch.setattr(
        daily_review,
        "_load_theme_snapshot_for_record",
        lambda **_kwargs: (
            {"status": "success", "path": "/tmp/theme.json"},
            _guard_snapshot()["theme_rotation"],
            _guard_snapshot(),
        ),
    )
    monkeypatch.setattr(
        daily_review,
        "_load_theme_shadow_monitor_for_record",
        lambda **_kwargs: {"status": "disabled"},
    )

    daily_review._attach_theme_overlay_to_record(run_dir, {"diagnostic_notes": []})
