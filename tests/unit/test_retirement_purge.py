...
import quant_investor.market.retirement as retirement_module
...
def test_initialize_rejects_symlink_and_hardlink_entries(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    root = repo / "results/v16"
    outside = tmp_path / "outside.txt"
    outside.write_text("outside", encoding="utf-8")
    (root / "unsafe-link").symlink_to(outside)
    with pytest.raises(RetirementError, match="non-regular entry"):
        initialize_cutover(
            repo_root=repo,
            journal_path=tmp_path / "private-1/journal.json",
            cutover_id="cutover-1",
            repo_sha256=REPO_SHA,
            skill_sha256=SKILL_SHA,
            schedule_sha256=SCHEDULE_SHAS,
    ...
    with pytest.raises(RetirementError, match="hard-linked file"):
        initialize_cutover(
            repo_root=repo,
            journal_path=tmp_path / "private-2/journal.json",
...
def test_full_purge_and_resume_receipt_do_not_preserve_deleted_bytes(
    ...
    assert not list((repo / "results").glob(".*.retirement-purge-*"))
...
def test_partial_tree_deletion_replays_only_after_durable_delete_started(
    tmp_path: Path,
) -> None:
    repo, journal, digest = _eligible(tmp_path)
    root = repo / "results/v16"
    ...
    def crash_after_first_entry(event: str, _root: object) -> None:
        nonlocal deleted_entries
        if event == "DELETE_ENTRY_UNLINKED":
            deleted_entries += 1
    ...
    with pytest.raises(SimulatedCrash):
        purge(
            journal_path=journal,
            expected_journal_sha256=digest,
            _event_hook=crash_after_first_entry,
            **_binding(repo),
        )

    crashed = load_journal(journal)
    assert crashed["roots"][0]["step"] == "DELETE_STARTED"
    assert not (repo / "results/v16").exists()
    final_sha = purge(
        journal_path=journal,
...
def test_delete_operational_failure_after_rename_is_durably_blocked(
    ...
    repo, journal, digest = _eligible(tmp_path)
    original_rmtree = retirement_module._safe_rmtree

    def fail_delete(*_args: object, **_kwargs: object) -> None:
        raise OSError("simulated unlink failure")

    monkeypatch.setattr(retirement_module, "_safe_rmtree", fail_delete)
    ...
    monkeypatch.setattr(retirement_module, "_safe_rmtree", original_rmtree)
    final_sha = purge(
        journal_path=journal,
        expected_journal_sha256=journal_sha256(journal),
