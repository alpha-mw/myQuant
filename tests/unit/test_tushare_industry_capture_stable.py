...
from quant_investor.market.tushare.contracts import response_projection_sha256
...
def test_plan_and_capture_replay_with_exact_official_cardinality() -> None:
    ...
    assert plan["kind"] == "market.tushare.industry_taxonomy_plan"
    assert capture["kind"] == "market.tushare.industry_taxonomy_capture"
    assert len(plan["contract_sha256"]) == 64
    assert len(capture["contract_sha256"]) == 64
...
def test_membership_plan_binds_all_l3_y_n_partitions() -> None:
    ...
    assert plan["kind"] == "market.tushare.industry_membership_plan"
    assert len(plan["contract_sha256"]) == 64
...
def test_membership_partition_capture_is_one_request_and_replayable() -> None:
    ...
    assert partition["kind"] == "market.tushare.industry_membership_partition"
    assert len(partition["contract_sha256"]) == 64
    assert (
        validate_industry_membership_partition_capture(
            partition,
            membership_plan=plan,
