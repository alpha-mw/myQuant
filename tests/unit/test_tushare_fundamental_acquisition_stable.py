...
from quant_investor.market.tushare.fundamental.models import (
    SOURCE_ENDPOINTS,
)
...
def test_full_partition_executor_closes_exact_keyset_without_fallback() -> None:
    closure = execution()
    client = FakeClient()
    result = acquire_fundamental_partitions(
    ...
    assert all(
        receipt["kind"] == "market.tushare.fundamental_partition_receipt"
        and len(receipt["contract_sha256"]) == 64
        and "version" not in receipt
...
def test_private_checkpoint_resume_replays_without_network(tmp_path: Path) -> None:
    ...
    first = acquire_fundamental_partitions(
        execution_closure=closure,
        client=client,
        captured_at=NOW,
    ...
    second = acquire_fundamental_partitions(
        execution_closure=closure,
