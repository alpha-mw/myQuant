...
def test_overlay_diagnostics_rejects_nonposterior_target() -> None:
    ...
    with pytest.raises(ValueError, match="target_name invalid"):
...
def test_overlay_and_model_schema_mismatches_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="schema_version invalid"):
...
def test_source_sha_covers_every_nested_posterior_field(
    ...
    if isinstance(value, float):
        ...
        setattr(container, field_name, value + ".mutated")
    else:
...
def test_shadow_rejects_legacy_calibration_schema_with_recomputed_proof() -> None:
    legacy_model = _model(_curve(CalibrationCurveKey()))
    legacy_model.schema_version = "2026-04-26.calibration-v2.legacy"
