...
SPEC = importlib.util.spec_from_file_location(
    "probe_tushare_10000_capabilities_stable",
    SCRIPT_PATH,
)
...
def test_token_parameter_and_public_surface_registration_are_forbidden() -> None:
    ...
    forbidden_import = "intelligence_" + "v2.sources.tushare"
    forbidden_runtime = "v17_" + "v4_runtime.tushare_https"
    for path in (
        ROOT / "scripts" / "probe_tushare_10000_capabilities.py",
        ROOT / "scripts" / "diagnose_tushare_endpoint_schema.py",
        ROOT / "scripts" / "capture_tushare_industry_taxonomy.py",
        ROOT / "scripts" / "capture_tushare_industry_membership.py",
        ROOT / "scripts" / "capture_tushare_theme_provider.py",
        ROOT / "scripts" / "run_tushare_vip_fundamental_shadow.py",
        ROOT / "scripts" / "run_tushare_vip_fundamental_upgrade.py",
    ):
        assert forbidden_import not in path.read_text()
        assert forbidden_runtime not in path.read_text()
...
def test_interrupt_is_not_reclassified_as_transport_failure() -> None:
    ...
    from quant_investor.market.tushare import probe_tushare_capabilities
