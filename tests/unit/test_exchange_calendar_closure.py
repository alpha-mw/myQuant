...
def _decoder_id(exchange: str, role: str) -> str:
    return f"test-only.native.{exchange.lower()}.{role.lower()}"
...
def _case(
    *,
    ...
    admission_by_subject: dict[tuple[str, str], dict[str, Any]] = {}
    sessions = [
        {
    ...
    for exchange in exchanges:
        ...
        projections["NOTICE_INDEX_SNAPSHOT"] = {
            "category": "TRADING_CALENDAR_NOTICES",
            "page_number": 1,
            "page_count": 1,
            "reported_item_count": 1,
            "window_start_date": coverage.isoformat(),
            "window_end_date": cutoff.isoformat(),
            "entries": [
                {
        ...
        for role, projection in projections.items():
            ...
            request_url = (
                ...
                else f"https://{HOSTS[exchange]}/calendar/{role.lower()}"
            )
            endpoint = (
                "/calendar/annual"
                if role == "ANNUAL_HOLIDAY_NOTICE"
            ...
            admission_by_subject[(exchange, role)] = admission
...
def _build(case: dict[str, Any]) -> dict[str, Any]:
    return build_exchange_calendar_compilation(
        ...
        created_at=BASE,
        decoder=_decoder,
        admission_resolver=lambda exchange, role: case["admission_by_subject"][(exchange, role)],
...
def test_native_closure_replays_three_exchanges_and_exact_outputs() -> None:
    ...
    assert (
        validate_exchange_calendar_compilation(
            ...
            decoder=_decoder,
            admission_resolver=lambda exchange, role: case["admission_by_subject"][
                (exchange, role)
            ],
            decoder_id_resolver=_decoder_id,
            decoder_sha256=DECODER_SHA,
