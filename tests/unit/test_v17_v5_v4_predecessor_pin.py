...
def test_git_object_pin_verifies_every_v4_asset_and_runtime_source() -> None:
    assert V4_SOURCE_GIT_COMMIT == "1da7ffb636a3254940525d746549d15e827f06ba"
    ...
    assert len(package["assets"]) + 1 == 102
    assert len(runtime["sources"]) == 31
...
def test_exact_merge_parent_and_v4_subtrees_preserve_sprint1c_identity() -> None:
    parents = subprocess.run(
        ["git", "show", "-s", "--format=%P", "0c6338f5706c9e236d06f943a2330d4770f64265"],
        cwd=ROOT,
        check=True,
        capture_output=True,
        text=True,
    ...
    assert parents.split() == [
        "f0173ec20ef49af5ca8ed8c4fbf962d75863fca0",
        V4_SOURCE_GIT_COMMIT,
    ]
...
def test_active_v2_and_explicit_legacy_v1_loaders_do_not_fallback() -> None:
    active = load_compatibility_policy()
    legacy = load_compatibility_policy_v1()
    assert active["version"] == "myquant.v17.v5.v4-compatibility-policy.v2"
...
def test_runtime_predecessor_verify_is_package_safe_without_git(tmp_path: Path) -> None:
    ...
    assert result["package_asset_count"] == 102
    assert result["runtime_source_count"] == 31


def test_missing_active_v2_policy_does_not_fall_back_to_valid_v1(tmp_path: Path) -> None:
    target = tmp_path / "v17_v5_contract"
    shutil.copytree(ROOT / "quant_investor/v17_v5_contract", target)
    (target / "resources/v4_compatibility_policy.v2.json").unlink()
