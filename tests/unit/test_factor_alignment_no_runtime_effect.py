...
def test_alignment_audit_helpers_are_absent_from_runtime_decision_modules() -> None:
    checked_paths = []
    ...
    for path in FORBIDDEN_RUNTIME_MODULES:
        ...
        for helper_name in ALIGNMENT_AUDIT_NAMES:
            assert helper_name not in text, f"{helper_name} leaked into {path}"

    assert checked_paths


def test_alignment_audit_does_not_touch_order_action_or_weight_paths() -> None:
    tracker_path = ROOT / "quant_investor" / "monitoring" / "cn_aggressive_portfolio_tracker.py"
    tracker_text = tracker_path.read_text(encoding="utf-8")

    assert "alignment_audit" not in tracker_text
    assert "FactorAlignmentAuditStore" not in tracker_text
    assert "audit_factor_backtest_alignment" not in tracker_text
    assert "alignment audit" not in tracker_text.lower()
