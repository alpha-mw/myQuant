...
from quant_investor.market.fundamental_provider_evidence._codec import (
    canonical_bytes,
    common_fields,
    content_ref,
    seal,
...
from quant_investor.market.fundamental_provider_evidence._comparison import (
    _build_legacy_fundamental_comparison_policy as build_legacy_fundamental_comparison_policy,
    build_fundamental_comparison_policy,
    compare_fundamental_raw_tables,
    validate_fundamental_comparison_policy,
)
from quant_investor.market.fundamental_provider_evidence._fileset import (
    REQUIRED_EVIDENCE_PATHS,
    build_provider_evidence_fileset_manifest,
    validate_provider_evidence_fileset_manifest,
)
from quant_investor.market.fundamental_provider_evidence._manifest import (
...
from quant_investor.market.fundamental_provider_evidence._reconciliation import (
    build_fundamental_reconciliation_receipt,
    validate_fundamental_reconciliation_receipt,
)
from quant_investor.market.fundamental_provider_evidence._storage import (
...
def policy() -> dict[str, Any]:
    return build_legacy_fundamental_comparison_policy(
        table_policies={
            table: {
...
def test_source_only_contract_rejects_every_unapproved_shape(
    ...
    with pytest.raises(FundamentalProviderEvidenceError):
        build_legacy_fundamental_comparison_policy(table_policies=table_policies, created_at=NOW)
...
def test_source_only_contract_does_not_allow_vip_or_unlisted_baseline_columns() -> None:
    ...
    with pytest.raises(FundamentalProviderEvidenceError, match="column order changed"):
        compare_fundamental_raw_tables(
            baseline_tables=baseline,
            vip_tables=vip,
            policy=forecast_source_only_policy(),
...
def test_provider_manifest_requires_passed_reconciliation_and_durable_fileset(
    ...
    manifest = build_fundamental_provider_manifest(
        **closure,
        created_at=NOW,
    )
    assert manifest["schema_version"] == "cn-fundamental-provider-manifest.v4"
    assert manifest["authoritative_full_rebuild"] is True
    assert validate_fundamental_provider_manifest(manifest, **closure) == manifest

    forged = copy.deepcopy(manifest)
    forged["vip_network_attempts"] = 7
    with pytest.raises(FundamentalProviderEvidenceError):
        validate_fundamental_provider_manifest(forged, **closure)
