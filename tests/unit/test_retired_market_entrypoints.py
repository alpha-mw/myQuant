...
RETIRED_PACKAGES = (
    f"quant_investor.{RETIRED_PROTOCOL}",
    f"quant_investor.bayesian.{RETIRED_PROTOCOL}",
)
...
def test_market_help_has_no_retired_commands_or_protocol_choice() -> None:
    parser = cli_main._build_parser()
    market = parser._subparsers._group_actions[0].choices["market"]
    choices = market._subparsers._group_actions[0].choices

    assert not any(name.startswith(RETIRED_PROTOCOL) for name in choices)
    assert not any(name.startswith("codex-review-") for name in choices)
    for command in ("analyze", "run"):
        protocol_action = next(
            action
            for action in choices[command]._actions
            if "--decision-protocol" in action.option_strings
        )
        assert protocol_action.choices == ["v15"]


@pytest.mark.parametrize("module_name", RETIRED_PACKAGES)
def test_retired_packages_are_absent_without_tombstones(module_name: str) -> None:
