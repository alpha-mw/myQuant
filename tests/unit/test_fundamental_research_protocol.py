...
PRIMARY_POLICY = SourceEligibilityPolicyV1(
    primary_hostnames={"example.com", "example.org"}
)
...
def test_positive_gate_and_primary_source_rules_fail_closed() -> None:
    ...
    secondary_request = request(
        source_policy_sha256=compute_source_policy_sha256(secondary_policy)
    )
...
def test_mixed_future_claim_and_claim_validity_cannot_drive_signal() -> None:
    ...
    historical_claim = next(
        item for item in mixed.claims if item.claim_id in financial.claim_ids
    )
    ...
    dimensions = [
        item.model_copy(update={"claim_ids": [historical_claim.claim_id, future_claim.claim_id]})
    ...
    unknown_mixed = mixed.model_copy(
        update={
            ...
            "dimensions": [
                item.model_copy(
                    update={
                        "claim_ids": [historical_claim.claim_id, unknown_claim.claim_id]
                    }
                )
                if item.dimension == Dimension.FINANCIAL_QUALITY
                else item
    ...
    for field, value in (
        ...
        invalid = baseline.model_copy(
            update={"claims": [first_claim, *baseline.claims[1:]]}
        )
        ...
        first = next(
            ...
            if item.dimension == first_claim.dimension
        )
