...
from quant_investor.fundamental_research.longitudinal_producer import (
    ...
    _load_analysis_manifest,
    _require_nav_attribution_basis,
...
def _control_chain_replay(*, variant: str, value: float) -> dict[str, object]:
    ...
    return {
        ...
        "variant": variant,
        "branch_summaries": {
            branch: {"final_score": 0.4, "final_confidence": 0.6}
        ...
        "branch_verdicts_by_symbol": {
            "600000.SH": {
                branch: {
                    ...
                    "metadata": (
                        fundamental_metadata if branch == "fundamental" else {}
                    ),
                }
                for branch in ("quant", "fundamental", "macro")
            }
...
def test_control_chain_replay_requires_exact_v15_branches_and_rejects_retired_keys():
    ...
    retired["branch_summaries"]["intelligence"] = {
        "final_score": 0.4,
        "final_confidence": 0.6,
    }
    with pytest.raises(ValueError, match="retired Intelligence key"):
        validate_control_chain_replay(retired)


def test_nav_attribution_rejects_two_empty_canonical_portfolios():
    with pytest.raises(LongitudinalProducerError, match="nonzero canonical target weight"):
        _require_nav_attribution_basis({"actual": {}, "counterfactual": {}})


def test_analysis_manifest_loader_rejects_supplied_symlink(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(
        "quant_investor.fundamental_research.governance.REPO_ROOT", tmp_path
    )
    target = tmp_path / "manifest.json"
    target.write_text("{}", encoding="utf-8")
    target.chmod(0o600)
    supplied = tmp_path / "manifest-link.json"
    supplied.symlink_to(target)

    with pytest.raises(LongitudinalProducerError, match="must not contain symlinks"):
        _load_analysis_manifest(supplied)
