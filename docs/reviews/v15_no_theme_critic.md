...
- Macro cutover correction: the live path is explicitly two-stage.
  `macro-maintain --authoritative-refresh` produces a hash-bound v15 control
  projection from a strict canonical observations generation, and
  `macro-promote` alone may CAS-publish it. Four-field compatibility and
  standalone-observation modes remain observer/staging only.
...
- The first provider-hard-off v15 run is successful when the contract is v15,
  Theme is absent and new risk remains blocked. Macro production readiness
  additionally requires a promoted, recomputable v15 control generation; an
  old v14 mart remains an explicit blocker.
