...
- CN Macro 正式补数入口：`quant-investor market macro-refresh --market CN --run-id <new_id> --expected-catalog-sha256 <sha> --expected-market-pointer-sha256 <sha> --nbs-cn-pmi-url <official_release_url> --allow-live`；只接受 latest-complete session。provider bundle v2 以国家统计局正式发布页作为 `cn_pmi` 主源，其余四个 endpoint 暂仍以 Tushare 为主源，并在 72 小时 capture window 内绑定 raw NBS sidecar、exact bar files 和版本化 transform，经共享 writer lock、双 CAS 与 journal 原子推进 strict catalog。`--allow-tushare-fallback` 默认关闭且只适用于已分类的 NBS 瞬态传输故障；周审 schedule 只能检查 help，不得运行该写入口。
